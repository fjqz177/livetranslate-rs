...
（本节随施工提交同步更新；测试基线、逐项落地情况、偏移记录见下。）

- 基线：454+7 → 本轮完成后以 `cargo test --workspace` 实测为准（见提交记录）。
- 有意偏离：**⑪（规则 4/5 偏离可见）本轮不做**——用户要求先解释清楚再定；
  已在代码注释与本节标注，未实现部分不含半成品。
- ⑬c（partial 快照格）**降级实现**：未引入 ArcSwap 快照格（跨 crate 共享成本高），
  改在生产者侧节流——队列容量压力显著缓解，但"单条翻译的字节量仍随文本增长"；
  若后续仍见动脉丢事件，再按 D-67 快照格模式升级。
