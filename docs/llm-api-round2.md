...
## 六、第三轮复核（2026-09-10，四只读子代理并行审计）与修复

四个子代理分头复核本轮成果（核心 crate / 编排+契约 / UI 配合面 / **对抗性打破尝试**），
主线程对全部 P1 逐条实证复现并修复。**对抗审计确实打破了三条承诺**——这正是复核的价值：

| 编号 | 发现 | 严重级 | 处理 |
|---|---|---|---|
| R2-15 | **流式模式下思维链仍泄漏**：`ReasoningStripper.visible` 只增不减，游离闭标签规则（GLM-5.3 形态）只在"整段一次喂入"时成立。实证：`预览…SECRET` + 游离闭标签 + 正文，批量给"正文"、**流式逐字给"预览…SECRET正文"**（默认就是流式） | P1 | ✗ 修：剥离器改以**累积原文为权威**（破坏性规则触发即整段重建、`finish` 由批量剥离产出）；新增回归测试 `streaming_strips_stray_close_shape` |
| R2-16 | **大小写变体标签泄漏**：`<Think>` / `<THINK>` / `<Thinking>` 全漏（实测） | P1 | ✗ 修：标签查找与块内闭合判定统一 ASCII 大小写不敏感；回归测试 `case_insensitive_tag_variants_are_stripped` |
| R2-17 | **「关不掉」误报并写坏配置**：官方保守端点（api.openai.com 等）起点就是"不发"，被判成"关不掉"→ 取消勾选 + 持久化标记 → 用户重勾又被取消（自证预言，永远发不出 `reasoning_effort:none`） | P1 | ✗ 修：回执判据加"**确实尝试过注入形态**"前提（`attempted_disable`）；测试扩到六组合 |
| R2-18 | **会话记忆不随配置失效**：记忆键 `(api_base, model)` 太粗——取消勾选后记忆仍注入关闭参数、改了密钥仍用退到底的最小请求 | P1 | ✗ 修：记忆与**配置指纹**绑定（改 disable_thinking/thinking_style/temperature/api_key/model/extra_body 等任一项即失效）；测试 `config_fingerprint_tracks_request_shape` |
| R2-19 | **最小请求不是最小**：`minimal()` 保留了 `json_response`，仍发 `response_format` + 改写系统提示词（老档案带着它时无路可退） | P1 | ✗ 修：`minimal()` 改**白名单构造**；`sanitize` 一并清理 `json_response`（与三个覆写键同理：界面看不见的不许影响实发）；测试 `minimal_drops_hidden_json_response` |
| R2-20 | 补发输出上限被拒后不降级（端点拒绝 `max_tokens` 时本可退到最小请求成功） | P2 | ✗ 修：补发失败后按同一套判据继续退级 |
...
错误体宽容解析是纯兜底（不比原路径差，只有在严格解析失败时才介入）、i18n 614 键 zh/en 1:1、
`⚠`/`—` 字形被内嵌字体覆盖（fontTools 实证）、回执落盘链路不依赖面板可见性。
