...
> **v1.3 修订**：四项裁决落地（默认关闭思考 / 记忆仅内存 / **UI 与后端一比一同步**（新增 §2.5 专章）/ 高级区全套保留）；复选框直接映射既有字段 `thinking_style`——**零新增契约字段**。
...
| 契约影响 | 纯新增 `UiEvent` 变体 2 个 + 枚举 3 个（加法豁免，`PROTO_VERSION` 不递增）；**`ModelConfig` 零新增字段** |
...
| # | 议题 | 裁决 | 落地位置 |
|---|---|---|---|
| 1 | "关闭模型思考"默认状态 | **默认关闭思考**（省 token、更快） | §2.3：默认 `thinking_style = auto`（既有默认值即为"自动关闭"），复选框默认勾选 |
...
| `proxy` | none | 不变 |
| **关闭模型思考**（复选框） | **勾选（关）** | **新增控件**，直接映射既有字段 `thinking_style`（§2.3） |
...
| 频率惩罚 / 存在惩罚 | 留空=不发 | 同上 |
| `context_turns` | 0 | 上下文轮数 |
| 无 system 角色 | 不勾 | 少数端点兼容开关 |
| `extra_body`（JSON） | 空 | 逃生舱：原样透传 |

### 2.3 「关闭模型思考」：一个复选框，**零新增字段**

**关键设计：复选框不引入新配置字段，而是直接映射既有的 `thinking_style`。**

| 界面状态 | 存储值 | 后端行为 |
|---|---|---|
| ☑ 关闭模型思考（默认） | `thinking_style = auto`（None，即现状默认值） | 走 §2.3.1 自动链，逐个试可用的关闭方式 |
| ☐ 不关闭 | `thinking_style = "off"` | 不发送任何推理相关参数，等模型想完取答案 |
| 高级区手动指定 | `"deepseek"` / `"qwen"` / `"vllm"` / `"openai"` | 只发该一种，不再自动试 |

因此：**读者只看到两个状态，而配置里始终只有一个字段**——这正是裁决 3 要求的"一比一"，且**契约零变更**（`THINKING_STYLES` 六值域原样保留，其中 `off` 即"未勾选"）。
...
+ 勾选"关闭模型思考"时的推理关闭参数（§2.3.1 自动链 / 手动指定）
```
...
| 波 | 内容 | 新增测试 | 验收判据 |
|---|---|---|---|
| **W1** | 接口与参数面 + **§2.5 的 a/b/c 三处同步修复**：`api.rs`；移除最大输出长度（内部不发）；关闭思考复选框（映射 `thinking_style`）+ 高级区全套；温度一等字段；悬浮窗模型下拉接线；`every_model_config_field_has_consumer` | +10 | 请求体不含 `max_tokens` 与预判参数；请求体预览与实发一致；悬浮窗换模型立即生效 |
...
| 面 | 处理 |
|---|---|
| 旧 `settings.json` | 无新增字段（§2.3 复用 `thinking_style`），故**无需 serde 迁移**；`json_response` / `overrides` 字段保留可读，界面不再展示 |
| 旧档案 `thinking_style` 的显示 | 如实反映：值为 `"off"` 的条目，复选框显示为**未勾选**（该值本就是用户显式选择）；用户勾上即切到自动关闭 |
