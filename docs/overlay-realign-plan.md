...
> 证据：`docs/ui-audit/40_rs_overlay_after.png`（Rust 实拍）、`41_rs_panel_vadasr.png`（全屏，可见悬浮窗压在桌面上无透视）、`42_side_by_side_headers.png`（两版头部并排）。
...
## 4. 实机双版同屏对照证据（2026-09-06 21:33，2560×1600 物理）

两版同机同屏运行（Python 原版经工作区副本 `.venv` 启动；Rust 为 release 重建后启动），全分辨率截图归档 `docs/ui-audit/`：

| 文件 | 内容 |
|---|---|
| `50_live_full_2133.png` | 双版同屏全屏原始帧 |
| `51_py_overlay_live.png` | 原版悬浮窗（**带实时消息**，虚拟声卡回放 F1 直播流） |
| `52_rs_overlay_live.png` | Rust 悬浮窗（空态，管线已启动） |
| `53/54_py/rs_header_live.png` | 两版头部原生分辨率特写 |
| `55_overlay_headers_compare.png` | 头部上下对照合成图 |

实拍坐实（与 §0/§1 推断一致）：

1. **透明度**：51 可透视底下窗口文字（RMS 37% 时仍透），52 为纯黑矩形把底下内容全部遮死——`opa()` 刻度 bug 的直观呈现。
2. **消息两行制**：51 中 `[21:33:47] [ko] あ。 ASR 158ms` 与下一行斜体 `翻译中...` 各占一行；同屏期 Rust 未能产出消息（见下），两行/混排对照待 Step B 喂假消息验收补拍。
3. **头部观感**：55 上下对照——Rust 按钮/下拉/复选均大一圈、下拉带大箭头按钮、电平条定宽右侧留白、全部字形为雅黑；原版按钮 20px 紧凑、下拉扁平 18px、电平条拉伸占满整行、chrome 全 Consolas。
