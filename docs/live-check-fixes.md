...
> **状态**：**施工中**（2026-09-17 定稿；决策登记 decisions.md **D-92**；实现提交见 §六）。
...
- A1 推导色（退出/暂停/字幕开/确认窗/字幕窗各族 hover+pressed）**首次落地，需实机逐屏目测**；不通过则只调常量，不动结构。
- B2a 的灰字会影响一切未显式着色的标签——施工时审计到的清单不保证穷尽，实机发现漏网按同法补。
- 走查单更正（`docs/drafts/1.0.0-live-check-batch1*.md`）：B1/B3/B4 三条按 §1.5 改写后再用。
