...
| 文档 | 状态 | 内容一句话 |
|---|---|---|
| `asr-chain-robustness.md` | 施工中 | 识别→翻译→显示链路健壮性修复：退出完整性（ACR-1a/1b/1c）+ 流式终态不可覆盖 + 字幕窗旁路账本 + 未就绪成对记账 + panic 补回执（D-94） |
| `doc-network-hardening.md` | 施工中 | 文档网络加固：清淤 + 守护断言 5~8 + 容量重校准（D-93/ADR-17） |
...
| `prompts/` | 开发流程卡（阶段自查清单；索引 = AGENTS §4 路由表；卡间顺序见各卡尾） | 项目终结（实际永久） |
