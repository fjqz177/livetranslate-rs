...
| `translator-probe-hotswap.md` | 供应商连接测试改造与热切换加固（2026-09-10 用户反馈 + 追问）：测试按钮四条卡死根因（UI 先锁态后发命令/命令走 ASR 线程空闲分支/目标三级静默回落/不可中断最坏 7 分钟）+ 多供应商链路取证（已实现，五处欠缺）+ 方案（每行测试按钮 / 独立探测线程 / 取消令牌 / 四态结果 / 热切换三加固 / 会话级累计费用）+ 精确接口与 i18n 键表 + 提交切分 + 验收清单；**D-85** 登记 | 方案定稿待施工（C1~C5 五提交） |
