...
| `architecture-v2.md` | 架构 2.0 方案：目标架构与迁移路线——五病灶诊断（谎言拓扑/字符串走私/设置镜像网/无监督线程树/UI 巨石）+ 12 项保留资产清单 + 七设计原则；十 crate 目标拓扑（新 lt-orchestrator / lt-download、lt-pipeline 改名 lt-audio）+ 线程监督器/事件动脉/arc-swap 设置总线三件套 + AppUi 拆分 + Boot v2 + 契约增删清单与冻结规则修订案；R1~R32 追溯矩阵 + W0~W7 八波绞杀迁移（各波验收标准可执行） | 方案定稿（2026-09-09，四路子代理专项取证 + 外部选型调研；待裁决开工） |
