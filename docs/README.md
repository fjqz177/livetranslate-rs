...
| `model-trust-repair.md` | 模型零信任校验与自动修复（2026-09-10 用户四条裁决）：加载前逐文件 sha256 闸门（启动/切换/唤醒三路径）+ 坏文件隔离 + 自动修复循环（3 次上限→提醒）+ 边界（指纹通过但加载失败不自动重下）+ 同轮评审勘误（"卡流永久挂起"证伪，30s 逐次读超时实测） | ✗ 施工中（**D-83** 登记；whisper sha256 20/20 前置已完） |
