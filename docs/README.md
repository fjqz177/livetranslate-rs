...
| `path-hygiene.md` | 硬编码路径全面排查与清理计划：多模式扫描 153 个已跟踪文本文件、P0×8（探针/测试常量个人路径 + docs 病灶引证）/P1×2（外部仓盘符）/P2×15（合成测试值与示例）分级证据表；探针路径 env 化（沿 LT_WHISPER_MODEL 先例）、合成路径 temp 派生、docs 脱敏（含归档只读脱敏例外）、守护脚本与提交切分 | ✗ 已施工（2026-09-09，PH-1~PH-5 落地；PH-6 参考图 GPU 中性化可选待裁决） |
...
| `architecture-v2.md` | 架构 2.0 方案：目标架构与迁移路线——五病灶诊断（谎言拓扑/字符串走私/设置镜像网/无监督线程树/UI 巨石）+ 12 项保留资产清单 + 七设计原则；十 crate 目标拓扑（新 lt-orchestrator / lt-download、lt-pipeline 改名 lt-audio）+ 线程监督器/事件动脉/arc-swap 设置总线三件套 + AppUi 拆分 + Boot v2 + 契约增删清单与冻结规则修订案；R1~R32 追溯矩阵 + W0~W7 八波绞杀迁移（各波验收标准可执行） | 方案定稿（2026-09-09，四路子代理专项取证 + 外部选型调研）；W0~W3 已合入，W4~W7 待施工 |
