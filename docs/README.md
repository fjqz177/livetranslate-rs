...
| `path-hygiene.md` | 硬编码路径全面排查与清理计划：多模式扫描 153 个已跟踪文本文件、P0×8（探针/测试常量个人路径 + docs 病灶引证）/P1×2（外部仓盘符）/P2×15（合成测试值与示例）分级证据表；探针路径 env 化（沿 LT_WHISPER_MODEL 先例）、合成路径 temp 派生、docs 脱敏（含归档只读脱敏例外）、守护脚本与提交切分 | 调研完成（2026-09-09；PH-1~PH-5 待施工，PH-6 参考图 GPU 中性化可选待裁决） |
