...
| D-73 | W6 | 二次启动激活已有窗口（WD-5）——已合入：singleton.rs（message-only 窗 `LT_SINGLETON_WND` + WM_APP 协议 + AllowSetForegroundWindow 前台锁防线 + proxy 投 SecondInstance；第二实例 `FindWindowW`→`PostMessageW`→退出 0）；lt-ui 消费=面板显示+SetForegroundWindow 前置 | 实机（二次启动双击→首实例面板前置）+ 协议常量单测 |
