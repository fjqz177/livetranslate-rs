...
- **模型下载链路改造（docs/download-overhaul.md，已完工 ✗）**：✗ DL-1 探测 manifest 化（e1b73ea）/ ✗ DL-2 下载器完整性+快速失败（96aab45）/ ✗ DL-3 进度协议精确化（dff0a5e）/ ✗ DL-4 会话化+取消+落盘归一（92c5d46，F9 同批）/ ✗ DL-5 hub 回落（17f5271）/ ✗ DL-6 杂项（14889a2）/ ✗ clippy 清理（6852da6）；334 测全绿 + release 构建过 + 实机冒烟真实缓存命中。新偏差 **D-22** 探测 manifest 化 / **D-23** 下载可取消（WP-B 届时自 D-24 起）。剩余：S1/S6 全程真实网络实机走查（需真实下载场景，可并入 WP-A）。
- **ASR 引擎扩展（docs/asr-engine-expansion.md，调研完成，阶段二新能力）**：WP-A FunASR Nano 实装 + 注册表修正（0.5–1 天，取代复刻期 WP-2 置灰防坑；前置 DL-1）→ WP-B Qwen3-ASR-0.6B spike + 实装（设置拓扑经用户拍板；新偏差落档 D-22 起）。
