...
- **模型下载链路改造（docs/download-overhaul.md，2026-09-08 审计定稿）**：审计出 13 项问题（2 高危：探测计入 .incomplete 致失败重试假成功死局 / nano 占位阈值致 MS 完整下载永判未缓存；1 失效：进度条解析 dead code）。施工卡 **DL-1~3（P0）** 探测 manifest 化/下载器完整性+快速失败/进度协议精确化 → **DL-4~5（P1）** 会话化下载线程+取消+落盘归一 / hub 缺失回落机制（D-21 机制半）→ **DL-6（P2）** 杂项；**DL-1 是 WP-A 前置**。新偏差提案 D-22 探测 manifest 化 / D-23 下载可取消（落地时回写归档决策史）。
