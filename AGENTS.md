...
- **ASR 引擎扩展（docs/asr-engine-expansion.md，r2 动工方案实测定稿 2026-09-08）**：WP-A FunASR Nano 实装（方案 §3 定稿：目标仓 `csukuangfj/sherpa-onnx-funasr-nano-int8-2025-12-30` 六文件清单 / **D-24 诚实下载源** = 无真实 MS 源禁止伪造 ms 字段、经 `hf_endpoint`（默认 hf-mirror）回退 / hf-mirror 实测 20.4MB/s、nano 模型已预置真实缓存；约 1 天，取代复刻期 WP-2 置灰防坑）→ WP-B Qwen3-ASR-0.6B spike + 实装（设置拓扑 B-α 经用户拍板；新偏差落档 **D-25 起**）。
