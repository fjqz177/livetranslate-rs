...
- **ASR 子系统加固（docs/asr-hardening.md，2026-09-08 五路审计定稿）**：21 项发现（H1 P0 + H2~H5 P1），施工包 **AH-1** 待命态可唤醒（P0：首启下载完成无法唤醒，`pipeline.rs:789-798` 不消费 tl_switch）/**AH-2** 恢复统一（间隙死亡 Status 预检不 recover + client=None 空窗 + shutdown kill，**D-26**）/**AH-3** ASR 线程运行时设置贯通（asr_language/pad 快照漂移可致字幕全灭）/**AH-4** interim 裁剪代际校验（原版同拓扑竞态，**D-27**）/**AH-5** 下载完整性收口（sha256 登记校验 + total=None 拒绝收尾 + 磁盘预检）/**AH-6** 分派臂收敛（data.rs 扫描漏 qwen3 已漂移实证 + hf_only 按注册表派生）/**AH-7** 可观测性五项（丢段计数/mic 重开/unavailable 边沿/worker subscriber/sherpa 根因）/**AH-8** qwen3 段长钳制 15s（**D-28**）+ nano set_language 超时对齐/**AH-9** fake worker 复用真实 `worker::run`（EngineFactory 已具备）+ 盲区用例/**AH-10** 文档纠偏（AGENTS 分层规则与 lt-ui 依赖矛盾、313/2 过时、SOURCES.md 三处）。新偏差自 **D-29** 起。施工顺序见该文档 §4。
