...
- **模型下载链路改造（docs/download-overhaul.md，已完工 ✗）**：✗ DL-1 探测 manifest 化（e1b73ea）/ ✗ DL-2 下载器完整性+快速失败（96aab45）/ ✗ DL-3 进度协议精确化（dff0a5e）/ ✗ DL-4 会话化+取消+落盘归一（92c5d46，F9 同批）/ ✗ DL-5 hub 回落（17f5271）/ ✗ DL-6 杂项（14889a2）/ ✗ clippy 清理（6852da6）；334 测全绿 + release 构建过 + 实机冒烟真实缓存命中。新偏差 **D-22** 探测 manifest 化 / **D-23** 下载可取消（后续新偏差自 D-25 起——D-24 已被 ASR WP-A 诚实下载源决策占用）。剩余：S1/S6 全程真实网络实机走查（HF 镜像段已随 WP-A 演练覆盖，2026-09-08）。
- **ASR 引擎扩展（docs/asr-engine-expansion.md，r2.1 动工方案实测定稿 2026-09-08）**：WP-A FunASR Nano 实装（方案 §3 定稿：目标仓 `csukuangfj/sherpa-onnx-funasr-nano-int8-2025-12-30` 六文件清单 / **D-24 诚实下载源** = 无真实 MS 源禁止伪造 ms 字段；HF 端点随所选 hub——选 HF=官方直连、选 MS=HF 尝试自动经 hf-mirror，不设设置键 / hf-mirror 实测 20.4MB/s、nano 模型已预置真实缓存；约 1 天，取代复刻期 WP-2 置灰防坑）→ WP-B Qwen3-ASR-0.6B spike + 实装（设置拓扑 B-α 经用户拍板；新偏差落档 **D-25 起**）。
