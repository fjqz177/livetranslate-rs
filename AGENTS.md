...
- **复刻权威 = 新版原版仓 `D:\biancheng\LiveTranslate`**（src/ + uv + 489 键 i18n）。工作区内 `LiveTranslate/`（gitignored）是原版代码副本。改 GUI 前先回读原版对应 Python 模块：`main.py`、`subtitle_overlay.py`、`subtitle_window.py`、`control_panel.py`、`vad_processor.py` 等。
