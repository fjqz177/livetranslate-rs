...
- **SenseVoice 语言恒 auto 修复（docs/sensevoice-language-fix.md，D-30）**：待施工 SV-1~SV-3——sherpa-onnx SenseVoice 输出无语言标签（实机证据），auto 时语言恒 "auto" → 「同语言免翻译」失灵（2026-09-08 用户反馈主诉）+ 显式语言时被语言过滤整段丢弃；方案 = 引擎层「显式设置 > 模型标签 > 启发式」三优先语言决策（与 nano/qwen3 共享 guess_language）。完成前新偏差自 **D-31** 起。
