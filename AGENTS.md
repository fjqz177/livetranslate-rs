...
- **悬浮窗行1按钮按压文字右移（docs/button-press-feedback.md，计划定稿）**：根因已实证（egui 0.36 `button_style` 内边距=`button_padding−bg_stroke.width` × stabilize 以 inactive 为基准（dark=0） × overlay_ui 只覆盖 inactive/hovered 未含 active → 按压右移 1.0 逻辑 px/200% DPI 2 物理 px，无头探针逐帧铁证）；**D-32 登记**；施工卡 BP-1 补 active 三态+按压底色（仅色变）、BP-2 stabilize 基准 `max(1.0)`、BP-3 widget_stability 扩展 active 用例（现有 hover 钉的漏网缺口）；按压反馈=仅底色加深（几何零变化）。
